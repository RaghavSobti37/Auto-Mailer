...
async function listPeople({ folder, search, page, limit, campaign, originSource, emailStatus, sort, order }) {
  ...
  if (folder === 'active') {
    query.$or = [{ opened: true }, { clicked: true }, { 'whatsapp.activityScore': { $gte: 3 } }];
  }

  if (folder === 'bounced') {
    query.$or = [{ bounced: true }, { 'whatsapp.status': 'failed' }];
  }
  ...
  if (folder === 'whatsapp_active') {
    query.channel = 'whatsapp';
    query['whatsapp.activityScore'] = { $gte: 3 };
  }

  if (campaign) {
    query.$or = [
      { campaignId: campaign },
      { 'whatsapp.campaignName': campaign },
      { 'whatsapp.campaignName': { $regex: campaign.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), $options: 'i' } },
    ];
  }
  ...
  if (folder === 'all' || folder === 'mail') {
    ...
    data = emailLogs.map((log) => ({
      _id: log._id,
      email: log.leadEmail?.endsWith('@whatsapp.local') ? '' : log.leadEmail,
      name: log.name || (log.leadEmail ? log.leadEmail.split('@')[0] : ''),
      phone: log.phone || '',
...
async function getFolderCounts() {
  ...
  const whatsapp = await EmailLog.countDocuments({ channel: 'whatsapp' });
  const whatsappActive = await EmailLog.countDocuments({ 'whatsapp.activityScore': { $gte: 3 } });
  ...
  return {
    folders: [
      ...
      { key: 'whatsapp_active', label: 'WA Active', count: whatsappActive },
      { key: 'campaigns', label: 'Campaigns', count: campaigns },
    ],
    counts: { all: totalPeople, active: opened, bounced, whatsapp, whatsappActive },
    groups: [],
  };
}
