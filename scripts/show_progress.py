#!/usr/bin/env python3
"""
Campaign progress tracking and reporting script.

Provides a consolidated progress report of email campaigns.
"""
...
MASTER_DB_PATH = os.path.join(BASE_DIR, 'data', 'csv', 'master_db_updated.csv')
EMAIL_LOG_PATH = os.path.join(BASE_DIR, 'data', 'csv', 'email_log.csv')


def show_campaign_progress():
    """Generate and display a consolidated campaign progress report."""
    ...
    if os.path.exists(EMAIL_LOG_PATH):
        try:
            ...
            if 'Timestamp' in df_log.columns and 'Status' in df_log.columns and 'EmailID' in df_log.columns:
                # Get unique sent and failed emails
                ...
                failed_emails = set(failed_df['EmailID']) - sent_emails  # Exclude later successes
                ...
                print(f"\n🟡 Warning: Log file is missing required columns.")
        except Exception as e:
            print(f"\n🟡 Warning: Could not process email log file: {e}")
    else:
        print(f"\n🟡 Info: Email log file not found. Stats will be based on master file only.")
