      ...
      <body className="min-h-screen">
        <QueryClientProvider client={queryClient}>
          <div className="flex h-screen overflow-hidden">
            <aside className={`${sidebarOpen ? 'w-60' : 'w-16'} flex shrink-0 flex-col border-r transition-all duration-200`} style={{ background: '#0b1220', borderColor: 'rgba(255,255,255,0.12)' }}>
              ...
              <nav className="flex-1 space-y-4 overflow-y-auto p-2">
                {['top', 'Compose', 'Reach', 'Insight'].map((group) => (
                  <div key={group} className="space-y-1">
                    {sidebarOpen && group !== 'top' && <div className="px-3 pt-2 text-[10px] font-semibold uppercase tracking-wider text-white/50">{group}</div>}
                    {NAV_ITEMS.filter((item) => item.group === group).map((item) => {
                      const isActive = pathname === item.href || (item.href !== '/' && pathname.startsWith(item.href));
                      return (
                        <Link key={item.href} href={item.href}
                          className={`relative flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition-colors ${isActive ? 'bg-white/12 text-white font-semibold' : 'text-white/78 hover:bg-white/8 hover:text-white'}`}>
